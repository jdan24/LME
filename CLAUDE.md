...
- "Monitor orders already in EMSX" (`PasteArea.tsx`) has an Active/Filled/All toggle (`MonitorFilter` in `types.ts`) that controls which EMSX statuses `App.tsx`'s `handleMonitorBlotter` pulls into the monitoring session — lets the trader choose rather than the app deciding for them. `handleRefreshOrders` and `handleSubmit`'s post-submit team-order merge intentionally stay on the older active+filled (exclude-dead-only) behavior and are **not** wired to this toggle.
